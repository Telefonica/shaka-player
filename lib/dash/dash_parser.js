...
shaka.dash.DashParser = class {
  ...
  constructor() {
    ...
    /**
     * Stores the last configuration for this.config_.dash.requestInterval.
     * @private {number}
     */
    this._lastRequestInterval_ = (this.config_ &&
      this.config_.dash.requestInterval) || 0;
    /** @private {shaka.util.Timer} */
    this.requestIntervalChecker_ = new shaka.util.Timer(() => {
      const requestInterval = (this.config_ &&
        this.config_.dash.requestInterval) || 0;
      if (this._lastRequestInterval_ !== requestInterval) {
        this._lastRequestInterval_ = requestInterval;
        this.onUpdate_();
      }
    });
    this.requestIntervalChecker_.tickEvery(
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    if (!this.manifest_) {
      let latencyTarget_ = -1;
      if (this.lowLatencyMode_) {
        const XmlUtils = shaka.util.XmlUtils;
        const serviceDescription = XmlUtils.findChild(mpd,
            'ServiceDescription');
        const latency = serviceDescription &&
          XmlUtils.findChild(serviceDescription, 'Latency');
        latencyTarget_ = (latency &&
          parseInt(latency.getAttribute('target'), 10)) ||
          0;
      }
      this.manifest_ = {
        presentationTimeline: presentationTimeline,
  ...
  setUpdateTimer_(offset) {
    ...
    const finalDelay = Math.max(
        ...
        this._lastRequestInterval_);

    shaka.log.debug('setUpdateTimer_', this._lastRequestInterval_,
        finalDelay);
