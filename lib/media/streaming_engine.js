...
shaka.media.StreamingEngine = class {
              ...
              .fullBox('tfdt', async (box) => {
                ...
                if (comparison1 < scaledMediaDecodeTime) {
                  ...
                  */
                  const streamsByType = new Map();
                  if (this.currentVariant_.audio) {
                    streamsByType.set(ContentType.AUDIO,
                        this.currentVariant_.audio);
                  }
                  if (this.currentVariant_.video) {
                    streamsByType.set(ContentType.VIDEO,
                        this.currentVariant_.video);
                  }
                  shaka.log.v1(logPrefix,
  ...
  async initSourceBuffer_(mediaState, reference) {
    ...
    if (timestampOffset != mediaState.lastTimestampOffset ||
      ...
      if ((mediaState.lastCodecs && codecs != mediaState.lastCodecs) ||
          (mediaState.lastMimeType && mimeType != mediaState.lastMimeType)) {
        if (mediaState.type === ContentType.VIDEO) {
          /** @type {shaka.media.StreamingEngine.MediaState_} */
          const audioState = this.mediaStates_.get(ContentType.AUDIO);
          if (audioState) {
            audioState.lastInitSegmentReference = null;
            this.abortOperations_(audioState);
          }
        } else if (mediaState.type === ContentType.AUDIO) {
          /** @type {shaka.media.StreamingEngine.MediaState_} */
          const videoState = this.mediaStates_.get(ContentType.VIDEO);
          if (videoState) {
            videoState.lastInitSegmentReference = null;
            this.abortOperations_(videoState);
          }
        }
      }
