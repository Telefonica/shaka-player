...
shaka.media.StreamingEngine = class {
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference) {
    ...
    try {
      ...
      const isFetchSupported = shaka.net.HttpFetchPlugin.isSupported();
      ...
      if (this.config_.lowLatencyMode && isFetchSupported && isMP4) {
            ...
            const fn = () => new Promise((resolve) => {
              if (remaining.byteLength) {
                ...
                if (sawMDAT) {
                  ...
                  busy = true;
                  shaka.log.info('mdat boxes', counter, presentationTime,
                      offset, remaining.byteLength);
        ...
        let remaining = shaka.util.BufferUtils.toUint8(result);
